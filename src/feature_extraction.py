...
def mfcc_feature_extraction(audio_path):
    """
    ...
    mfccs = mfcc(signal, samplerate=sr, winlen=winlen, winstep=winstep, numcep=numcep, nfft=nfft, nfilt=nfilt)
...
def compute_histograms(audio, base, n_freq = 1023):
    """
...
def first_digit_call(dataset_path, base):
    ...
    for i, (dirpath, dirnames, filenames) in enumerate(os.walk(dataset_path)):
        ...
        for f in filenames:
            ...
            audio_mfccs = mfcc_feature_extraction(audio_path)
            ...
            audio_mfccs = audio_mfccs[:, 1:]  # shape (numframes, 1023)

            # actually compute first digit vector
            fd = first_digit_gen(audio_mfccs, base)

            # computing histograms
            ff = compute_histograms(fd, base) # matrix with shape (frequencies, probabilities) = (1023, 9)
...
if __name__ == '__main__':
    ...
    df.to_csv("datasets/df_jsut_multiband_melgan.csv")
